...
apply_gtklock() {
    ...
    if [ ! -f "scripts/templates/gtklock/main.scss" ]; then
        ...
        sassc "$CONFIG_DIR"/scripts/templates/gtklock/main.scss ~/.config/gtklock/style.css
        return
    fi
...
apply_term() {
    ...
    if [ ! -d "$CACHE_DIR/colorschemes" ]; then
        mkdir -p "$CACHE_DIR/colorschemes"
    fi
    # Copy template
    cp "scripts/templates/terminal/sequences.material" "$CACHE_DIR/colorschemes/sequences"
    # Apply colors
    for i in "${!colorlist[@]}"; do
        sed -i "s/${colorlist[$i]} #/${colorvalues[$i]#\#}/g" "$CACHE_DIR/colorschemes/sequences"
    done

    for file in /dev/pts/*; do
      if [[ $file =~ ^/dev/pts/[0-9]+$ ]]; then
        cat "$CACHE_DIR/colorschemes/sequences" > "$file"
      fi
    done
}

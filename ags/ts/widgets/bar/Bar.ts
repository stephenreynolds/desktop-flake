...
export default (monitor = 0) => {
  ...
  const setFloating = (box) =>
    Utils.timeout(15, async () => {
      try {
        ...
        const clients = Hyprland.clients.filter(
          (c) => c.workspace.id === workspace.id && c.mapped,
        );
        ...
        if (workspace.windows === 1) {
          ...
          const noGapsWhenOnly = JSON.parse(
            await Utils.execAsync([
              "hyprctl",
              "getoption",
              `${layout}:no_gaps_when_only`,
              "-j",
            ]),
          ).int;
          if (noGapsWhenOnly === 1) {
        ...
        const workspacerules = JSON.parse(
          await Utils.execAsync(["hyprctl", "workspacerules", "-j"]),
        );
        const barWorkspace = workspacerules.find(
          (rule) => rule.workspaceString === activeWorkspace.toString(),
        );
        if (!barWorkspace) {
          return;
        }
  ...
  const barContent = Widget.CenterBox({
    ...
    // setup: (self) =>
    //   self.hook(App, setFloating, "config-parsed").hook(
    //     Hyprland,
    //     (self, event) => {

...
function setGaps(gapsout: number, gapsin: number) {
  return Hyprland.workspaces.map((workspace) => {
    const tiledClients = Hyprland.clients.filter(
      (c) => c.workspace.id === workspace.id && !c.floating && c.mapped,
    );

    const noGapsWindowClasses = options.hyprland.gaps.noGapsWindowClasses.value;

    if (
      tiledClients.length > 0 &&
      tiledClients.every((c) => noGapsWindowClasses.includes(c.class))
    ) {
      Hyprland.messageAsync(
        `keyword workspace ${workspace.id},gapsout:0,gapsin:0,rounding:false,border:false`,
      ).catch(() => {});
      return;
    }

    Hyprland.messageAsync(
      `keyword workspace ${workspace.id},gapsout:${gapsout},gapsin:${gapsin},rounding:true,border:true`,
    ).catch(() => {});
  });
}
...
function onCloseWindow(client: Client | undefined) {
  ...
  if (
    !activeWorkspace ||
    activeWorkspace.windows > 1 ||
    clientInSpecial(client)
  ) {
    return;
  }

  if (activeWorkspace.windows === 1) {
    const lastClient = Hyprland.clients.find(
      (c) => c.workspace.id === activeWorkspace.id,
    );
    ...
    activeWorkspace.windows === 0 &&
    client &&
    ["Picture-in-Picture", "Launching..."].includes(client.title)
  ) {
    return;
  }

  const isLastWorkspaceOnMonitor =
    Hyprland.workspaces.filter(
      (w) => w.monitorID === Hyprland.active.monitor.id && w.id !== -99,
    ).length === 1;
...
export async function setupHyprland() {
  ...
  batch.push(
    ...
    `master:no_gaps_when_only ${options.hyprland.gaps.noGapsWhenOnly.value}`,
    `dwindle:no_gaps_when_only ${options.hyprland.gaps.noGapsWhenOnly.value}`,
  );
